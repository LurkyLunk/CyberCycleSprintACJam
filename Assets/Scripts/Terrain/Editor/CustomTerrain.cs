...
public class CustomTerrain : MonoBehaviour
{
                ...
                foreach (Vector2 n in neighbours)
                {
                    if (heightMap[x, y] < waterHeight && heightMap[(int)n.x, (int)n.y] > waterHeight)
                    {
                        ...
                        go.transform.position = this.transform.position + 
                                        new Vector3(n.y / (float)terrainData.heightmapResolution
                                                        * terrainData.size.z,
                                                    waterHeight * terrainData.size.y,
    ...
    public void SplatMaps()
    {
        ...
        foreach (SplatHeights sh in splatHeights)
        {
            ...
            terrainLayer[terrainIndex].diffuseTexture = sh.diffuseTexture;
            ...
            string path = "Assets/" + this.gameObject + "TerrainLayer" + 
            terrainIndex + ".terrainlayer";
        ...
        float[,,] terrainmapData = new float[terrainData.alphamapWidth, terrainData.alphamapHeight, terrainData.alphamapLayers];
        ...
        for (int y =0; y <terrainData.alphamapHeight; y++)
        {
            for (int x = 0; x < terrainData.alphamapWidth; x++)
            {
                ...
                {
                    terrainmapData[x, y, j] = splat[j];
                }
            }
        }
        terrainData.SetAlphamaps(0, 0, terrainmapData);
    }
    ...
    public void MidPointDisplacement()
    {
        ...
        heightMap[terrainData.heightmapWidth - 1, terrainData.heightmapHeight - 1] =
